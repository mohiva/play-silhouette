...
abstract class OAuth2Provider(application: Application, jsonResponse: Boolean = true) extends IdentityProvider(application) {
  ...
  private def getAccessToken[A](code: String)(implicit request: Request[A]):OAuth2Info = {
    ...
    try {
      ...
      case e: Exception => {
        Logger.error("[silhouette] error trying to get an access token for provider %s".format(id), e)
  ...
  protected def buildInfo(response: Response): OAuth2Info = {
      ...
      if ( Logger.isDebugEnabled ) {
        Logger.debug("[silhouette] got json back [" + json + "]")
      }
      OAuth2Info(
  ...
  def doAuth[A]()(implicit request: Request[A]): Either[Result, SocialUser] = {
    request.queryString.get(OAuth2Constants.Error).flatMap(_.headOption).map( error => {
      error match {
        ...
        case _ =>
          Logger.error("[silhouette] error '%s' returned by the authorization server. Provider type is %s".format(error, id))
          throw new AuthenticationException()
      }
      throw new AuthenticationException()
    })
    ...
    request.queryString.get(OAuth2Constants.Code).flatMap(_.headOption) match {
      case Some(code) =>
        ...
        if ( Logger.isDebugEnabled ) {
          Logger.debug("[silhouette] user = " + user)
        }
        user match  {
          case Some(u) => Right(u)
      ...
      case None =>
        ...
        if ( Logger.isDebugEnabled ) {
          Logger.debug("[silhouette] authorizationUrl = %s".format(settings.authorizationUrl))
          Logger.debug("[silhouette] redirecting to: [%s]".format(url))
        }
