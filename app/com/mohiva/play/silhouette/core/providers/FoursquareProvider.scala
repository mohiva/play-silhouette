/**
 * Copyright 2014 Brian Porter (poornerd at gmail dot com) - twitter: @poornerd
  ...
  def fillProfile(user: SocialUser): SocialUser = {
    ...
    try {
      ...
      (me \ "response" \ "user").asOpt[String] match {
        case Some(msg) => {
          Logger.error("[silhouette] error retrieving profile information from Foursquare. Message = %s".format(msg))
      ...
      case e: Exception => {
        Logger.error( "[silhouette] error retrieving profile information from foursquare", e)
        throw new AuthenticationException()
      }
    }
  }
