...
class GitHubProvider(application: Application) extends OAuth2Provider(application) {
  ...
  override protected def buildInfo(response: Response): OAuth2Info = {
    ...
    if ( accessToken.isEmpty ) {
      Logger.error("[silhouette] did not get accessToken from %s".format(id))
  ...
  def fillProfile(user: SocialUser): SocialUser = {
    ...
    try {
      ...
      (me \ Message).asOpt[String] match {
        case Some(msg) => {
          Logger.error("[silhouette] error retrieving profile information from GitHub. Message = %s".format(msg))
          throw new AuthenticationException()
        }
        case _ => {
      ...
      case e: Exception => {
        Logger.error( "[silhouette] error retrieving profile information from github", e)
        throw new AuthenticationException()
      }
    }
  }
