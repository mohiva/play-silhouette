    ...
    try {
      ...
      (me \ Error).asOpt[JsObject] match {
        case Some(error) =>
          ...
          Logger.error("[silhouette] error retrieving profile information from Google. Error type = %s, message = %s"
      ...
      case e: Exception => {
        Logger.error( "[silhouette] error retrieving profile information from Google", e)
        throw new AuthenticationException()
      }
    }
  }
