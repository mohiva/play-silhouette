...
abstract class PasswordHasher extends Plugin with Registrable {
  ...
  override def onStart() {
    Logger.info("[silhouette] loaded password hasher %s".format(id))
    Registry.hashers.register(this)
  }


  override def onStop() {
    Logger.info("[silhouette] unloaded password hasher %s".format(id))
...
class BCryptPasswordHasher(app: Application) extends PasswordHasher {
  ...
  val RoundsProperty = "silhouette.passwordHasher.bcrypt.rounds"
