...
class FacebookProvider(application: Application) extends OAuth2Provider(application) {
  ...
  override protected def buildInfo(response: Response): OAuth2Info = {
    response.body.split("&|=") match {
        ...
        case _ =>
          Logger.error("[silhouette] invalid response format for accessToken")
    ...
    try {
      ...
      (me \ Error).asOpt[JsObject] match {
        case Some(error) =>
          ...
          Logger.error(
            "[silhouette] error retrieving profile information from Facebook. Error type: %s, message: %s".
      ...
      case e: Exception => {
          Logger.error("[silhouette] error retrieving profile information from Facebook",  e)
          throw new AuthenticationException()
      }
    }
  }
