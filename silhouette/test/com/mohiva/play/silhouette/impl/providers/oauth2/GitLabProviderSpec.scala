...
class GitLabProviderSpec extends OAuth2ProviderSpec {
  ...
  "The `retrieveProfile` method" should {
    "fail with ProfileRetrievalException if API returns error" in new WithApplication with Context {
      val requestHolder = mock[WSRequest]
      val response = mock[WSResponse]
      response.json returns Helper.loadJson("providers/oauth2/gitlab.error.json")
    ...
    "use the overridden API URL" in new WithApplication with Context {
      ...
      response.json returns Helper.loadJson("providers/oauth2/gitlab.success.json")
      requestHolder.get() returns Future.successful(response)
  ...
  trait Context extends OAuth2ProviderSpecContext {
     ...
     * The OAuth2 settings.
     */
    override lazy val oAuthSettings = spy(OAuth2Settings(authorizationURL = Some("https://gitlab.com/oauth/authorize"), accessTokenURL = "https://gitlab.com/oauth/token", redirectURL = Some("https://www.mohiva.com"), clientID = "my.client.id", clientSecret = "my.client.secret", scope = Some("api")))
