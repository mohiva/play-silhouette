...
/**
 * Test case for the [[JWTAuthenticator]].
 */
class JWTAuthenticatorSpec extends PlaySpecification with Mockito with JsonMatchers with NoLanguageFeatures {
  ...
  "The `serialize` method of the authenticator" should {
    "return a JWT with an expiration time" in new WithApplication with Context {
      val jwt = serialize(authenticator, authenticatorEncoder, settings, jwtAlgorithmSignerFactory)
    ...
    "throw an AuthenticatorException if a reserved claim will be overridden" in new WithApplication with Context {
      ...
      serialize(authenticator.copy(customClaims = Some(claims)), authenticatorEncoder, settings, jwtAlgorithmSignerFactory) must throwA[AuthenticatorException].like {
    ...
    "return a JWT with arbitrary claims" in new WithApplication with Context {
      val jwt = serialize(authenticator.copy(customClaims = Some(customClaims)), authenticatorEncoder, settings, jwtAlgorithmSignerFactory)
      val json = Base64.decode(jwt.split('.').apply(1))
  ...
  "The `unserialize` method of the authenticator" should {
    "throw an AuthenticatorException if the given token can't be parsed" in new WithApplication with Context {
      ...
      unserialize(jwt, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory) must beFailedTry.withThrowable[AuthenticatorException](msg)
    }

    "throw an AuthenticatorException if the given token couldn't be verified" in new WithApplication with Context {
      val jwt = serialize(authenticator, authenticatorEncoder, settings, jwtAlgorithmSignerFactory) + "-wrong-sig"
      val msg = Pattern.quote(InvalidJWTToken.format(ID, jwt))
    ...
    "unserialize a JWT" in new WithApplication with Context {
      ...
      unserialize(jwt, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory) must beSuccessfulTry.withValue(authenticator.copy(
    ...
    "unserialize a JWT with arbitrary claims" in new WithApplication with Context {
      ...
      unserialize(jwt, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory) must beSuccessfulTry.like {
        case a =>
  ...
  "The `serialize/unserialize` method of the authenticator" should {
    "serialize/unserialize an authenticator" in new WithApplication with Context {
      ...
      unserialize(jwt, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory) must beSuccessfulTry.withValue(authenticator)
    }
  }
  ...
  "The `retrieve` method of the service" should {
    ...
    "return authenticator if DAO is enabled and an authenticator is stored for the token located in the the header" in new WithApplication with Context {
      implicit val request = FakeRequest().withHeaders(settings.fieldName -> serialize(authenticator, authenticatorEncoder, settings, jwtAlgorithmSignerFactory))
    ...
    "return authenticator if DAO is enabled and an authenticator is stored for the token located in the the query string" in new WithApplication with Context {
      implicit val request = FakeRequest("GET", s"?${settings.fieldName}=${serialize(authenticator, authenticatorEncoder, settings, jwtAlgorithmSignerFactory)}")
  ...
  "The `init` method of the service" should {
    "return the token if DAO is enabled and authenticator could be saved in backing store" in new WithApplication with Context {
      ...
      unserialize(token, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory).get must be equalTo authenticator
  ...
  "The result `embed` method of the service" should {
    "return the response with a header" in new WithApplication with Context {
      implicit val request = FakeRequest()
      val token = serialize(authenticator, authenticatorEncoder, settings, jwtAlgorithmSignerFactory)
  ...
  "The request `embed` method of the service" should {
    "return the request with a header " in new WithApplication with Context {
      ...
      unserialize(request.headers.get(settings.fieldName).get, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory).get must be equalTo authenticator
    }
  ...
  "The `update` method of the service" should {
    ...
    "return the result if the authenticator could be stored in backing store" in new WithApplication with Context {
      ...
      unserialize(header(settings.fieldName, result).get, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory).get must be equalTo authenticator
      there was one(repository).update(authenticator)
    }
  ...
  "The `renew` method of the service" should {
    "renew the authenticator and return the response with a new JWT if DAO is enabled" in new WithApplication with Context {
      ...
      unserialize(header(settings.fieldName, result).get, authenticatorEncoder, settings, jwtAlgorithmVerifierFactory).get must be equalTo authenticator.copy(
        id = id,
  ...
  trait Context extends Scope {
    ...
    lazy val jwtAlgorithmVerifierFactory: JWTAlgorithmVerifierFactory = new MACAlgorithmFactory(JWSAlgorithm.HS256)
    lazy val jwtAlgorithmSignerFactory: JWTAlgorithmSignerFactory = new MACAlgorithmFactory(JWSAlgorithm.HS256)
    /**
     * Some custom claims.
     */
